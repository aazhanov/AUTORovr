...
def direction_determination():
    ...
    if min_dist_sensor == 'M':  # middle cases, higher priority, check first
        ...
        print('ERROR: No case found for direction_determination')
    return direction
...
def oa_threshold(threshold_dist=30): # from testing 50 seemed short, trying 100
    ...
    sensors_less_than_thresh = np.any(sensor_check < threshold_dist) 
    if sensors_less_than_thresh:
        print('Threshold exceeded!')
    #return np.any(sensor_check < threshold_dist)
    return sensors_less_than_thresh
...
def execute_command(direction, duration, ser):
    ...
    if direction == "fwd":
        ...
        print("checking obstacle avoidance loop")
        while duration >= 0:
            time.sleep(0.01)
            duration -= 0.01
            # check for OA here
            if oa_threshold():  # check for obstacle
                ...
                return True
            process_time_for_OA = 0.725  # process time for this function is 0.725987 seconds
            duration -= process_time_for_OA  # subtract the process time for OA to account for time lost during oa_threshold
        ser.write(b'\x00')
        print("obstacle not detected during duration")# stops motors
        return False
...
def test(ser):
    while(oa_threshold()):
        ...
        if angle > 0:  # positive angle check
           execute_command('right', angle, ser)
           print('DEBUG 370: Turning right')
           print(f"turning {angle} degrees right")
        elif angle < 0:  # negative angle check
           angle *= -1
           execute_command('left', angle, ser)
           print('DEBUG 374: Turning left')
...
#start = time.time()
#boolian = oa_threshold()
#end = time.time()
#difference = end - start
#print(difference)
#node_seeker(gps, ser, datapath)
#execute_command('fwd', 2, ser)
#print(execute_command('fwd', 1, ser))
direction_degree_dict = {'hard left': -18, 'left': -13, 'right': 13, 'hard right': 18,
                                 'mid hard left': -22, 'mid left': -20, 'mid right': 20, 'mid hard right': 22}
